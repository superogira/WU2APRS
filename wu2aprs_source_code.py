...
# -*- coding: utf-8 -*-
"""
WU2APRS 2.3 — LOCAL data from JSON endpoint + hotkey toggle
Hotkeys:
  W : send WX now
  S : send Status now
  R : toggle RF (AFSK) ON/OFF
  I : toggle IS (APRS-IS) ON/OFF
  D : toggle DATA SOURCE (WU ↔ LOCAL)
  U : force DATA SOURCE = WU
  L : force DATA SOURCE = LOCAL
...
[local]
ip = 192.168.1.158
json_path = /client?command=record
protocol = http
json_url =           ; (optional) full override. If set, ip/protocol/json_path are ignored.
"""

import os, sys, time, socket, math, threading, json
import configparser
...
# Unit conversions (LOCAL → imperial-like fields)
def c_to_f(c): return float(c) * 9.0/5.0 + 32.0
def ms_to_mph(ms): return float(ms) * 2.2369362921
def hpa_to_inhg(h): return float(h) / 33.8638866667
def mm_to_in(mm): return float(mm) / 25.4

# =========================
#   WX builders & fetchers
# =========================

def build_wx_string(obs):
    """
    obs may include (imperial-like units):
      winddir (deg 0–360)
      windSpeed (mph) — 2-min avg if available
      windGust (mph)
      temp (F)
      humidity (%)
      pressure (inHg)
      precipTotal (inches since midnight)
      precipHour (inches last 1 hour)  [optional]
      precip24h  (inches last 24 hours)[optional]
    ...
    precip_hour_in  = obs.get("precipHour")
    precip_24h_in   = obs.get("precip24h")
    ...
    r_field = inches_to_hundredths(precip_hour_in, width=3) if precip_hour_in is not None else "000"
    p_field = inches_to_hundredths(precip_24h_in,  width=3) if precip_24h_in  is not None else "000"
    ...
    b_field = inhg_to_tenths_mbar(pressure_in)  # tenths of millibars
...
def _sensor_section(sensor_list, title_prefix):
    for sec in sensor_list:
        t = str(sec.get("title","")).strip().lower()
        if t.startswith(title_prefix.lower()):
            return sec
    return None

def _get_named_value(sec, name):
    ...
    for item in sec.get("list", []):
        ...
        if label == name.lower():
            try:
                return float(str(item[1]).strip())
            except Exception:
                return None
    return None

def fetch_local_observation_json(local_json_url):
    r = requests.get(local_json_url, timeout=10)
    r.raise_for_status()
    data = r.json()

    sensors = data.get("sensor", []) if isinstance(data, dict) else []
    sec_outdoor = _sensor_section(sensors, "outdoor")
    sec_pressure = _sensor_section(sensors, "pressure")
    sec_wind = _sensor_section(sensors, "wind speed")
    sec_rain = _sensor_section(sensors, "rainfall")
    sec_solar = _sensor_section(sensors, "solar")

    temp_c = _get_named_value(sec_outdoor, "Temperature")
    hum_out = _get_named_value(sec_outdoor, "Humidity")

    rel_hpa = _get_named_value(sec_pressure, "Relative")
    if rel_hpa is None:
        rel_hpa = _get_named_value(sec_pressure, "Absolute")

    wind_avg2_ms = _get_named_value(sec_wind, "Wind Average 2 Minute")
    wind_ms      = _get_named_value(sec_wind, "Wind")
    gust_ms      = _get_named_value(sec_wind, "Gust")
    dir_avg2_deg = _get_named_value(sec_wind, "Direction Average 2 Minute")
    dir_deg      = _get_named_value(sec_wind, "Direction")

    rain_hour_mm = _get_named_value(sec_rain, "Hour")
    rain_day_mm  = _get_named_value(sec_rain, "Day")

    light_wm2 = _get_named_value(sec_solar, "Light")

    winddir = dir_avg2_deg if dir_avg2_deg is not None else dir_deg
    base_ms = wind_avg2_ms if wind_avg2_ms is not None else wind_ms
    wind_mph = ms_to_mph(base_ms) if base_ms is not None else None
    gust_mph = ms_to_mph(gust_ms) if gust_ms is not None else None

    tempF = c_to_f(temp_c) if temp_c is not None else None
    pressure_in = hpa_to_inhg(rel_hpa) if rel_hpa is not None else None
    ...
    return {
        ...
        "precipHour": mm_to_in(rain_hour_mm) if rain_hour_mm is not None else None,
        "precipTotal": mm_to_in(rain_day_mm) if rain_day_mm is not None else None,  # since midnight ≈ Day
        "solarRadiation": light_wm2,
    }
...
def do_send_wx(cfg, obs_fetcher, position_block, device_str, dev_in_comment, meta_in_comment,
...
def main():
    ...
    cfg.read("config_2.3.ini", encoding="utf-8")
    ...
    sw_tag   = cfg.get("aprs", "software_tag", fallback="WU2APRS 2.3")
    ...
    wu_url = ( "https://api.weather.com/v2/pws/observations/current"
               f"?stationId={wu_station}&format=json&units={wu_units}&apiKey={wu_key}" ) if (wu_station and wu_key) else None

    # LOCAL JSON URL
    local_json_url = cfg.get("local", "json_url", fallback="").strip()
    if not local_json_url:
        proto = cfg.get("local", "protocol", fallback="http").strip(":/")
        ip    = cfg.get("local", "ip", fallback="192.168.1.158").strip()
        path  = cfg.get("local", "json_path", fallback="/client?command=record")
        if not path.startswith("/"):
            path = "/" + path
        local_json_url = f"{proto}://{ip}{path}"

    # Data source
    source_mode = cfg.get("source", "mode", fallback="wu").strip().lower()
    if source_mode not in ("wu","local"):
        source_mode = "wu"
    ...
    print(f"Starting WU2APRS: {callsign} | SRC={source_mode.upper()} | IS={'on' if is_on else 'off'} "
    ...
    hk_tog_src = threading.Event()
    hk_force_wu = threading.Event()
    hk_force_local = threading.Event()
    ...
    def _hotkey_thread():
        ...
        if not hotkeys_enabled:
            return
        if hotkeys_hint:
            print("[hotkeys] W=WX, S=Status, R=toggle RF, I=toggle IS, D=toggle SOURCE (U=WU, L=LOCAL). Q=quit console mode.")
        mode = hotkeys_mode
        if mode == "auto":
            try:
                ...
                keyboard.add_hotkey('d', lambda: hk_tog_src.set())
                keyboard.add_hotkey('u', lambda: hk_force_wu.set())
                keyboard.add_hotkey('l', lambda: hk_force_local.set())
        ...
        if mode == "console":
            if os.name == 'nt':
                try:
                    ...
                    print("[hotkeys] Console mode: press keys in this window (W/S/R/I/D/U/L/Q).")
                    while True:
                        if msvcrt.kbhit():
                            ...
                            elif ch == 'd': hk_tog_src.set()
                            elif ch == 'u': hk_force_wu.set()
                            elif ch == 'l': hk_force_local.set()
                            elif ch == 'q': break
                        time.sleep(0.05)
                except Exception as e:
    ...
    while True:
        try:
            ...
            if hk_force_wu.is_set():
                hk_force_wu.clear()
                source_mode = "wu"
                print(f"[{_nowz()}] SET SOURCE: WU")
            if hk_force_local.is_set():
                hk_force_local.clear()
                source_mode = "local"
                print(f"[{_nowz()}] SET SOURCE: LOCAL")
            if hk_tog_src.is_set():
                hk_tog_src.clear()
                source_mode = "local" if source_mode == "wu" else "wu"
                print(f"[{_nowz()}] TOGGLE SOURCE: {source_mode.upper()}")

            # Build fetcher with current source_mode
            def obs_fetcher():
                if source_mode == "local":
                    return fetch_local_observation_json(local_json_url)
                else:
                    if not wu_url:
                        raise RuntimeError("WU source selected but station_id/api_key missing in config.ini [wu].")
                    return fetch_wu_observation(wu_url)

            # Handle hotkeys first (immediate TX)
            if hk_wx.is_set():
                hk_wx.clear()
                try:
                    do_send_wx(cfg, obs_fetcher, position_block, device_str, dev_in_comment, meta_in_comment,
                               is_on, rf_on, server, port, callsign, passcode, sw_tag, tocall, log_preview, log_maxlen)
                except Exception as e:
